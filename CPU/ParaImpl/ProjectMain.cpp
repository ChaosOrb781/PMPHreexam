...
typedef int (*funT)<int>(const uint, const uint, const uint, const uint, const REAL, const REAL, const REAL, const REAL, const REAL, REAL*);
...
template <typename ftype, int blocksize = 0>
ReturnStat* RunStatsOnProgram(const char* name, ftype f, 
...
void RunTestOnProgram(const char* title, fun f, REAL* expected, ReturnStat* expectedStats, const uint outer, const uint numX, const uint numY, const uint numT,
	...
	ReturnStat* returnstatus = RunStatsOnProgram<ftype, blocksize>(title, f, res, outer, numX, numY, numT, s0, t, alpha, nu, beta);
...
int main()
{
    ...
    if (is_valid) {
        ...
        RunTestOnProgram<fun>("Simple Parallel", run_SimpleParallel, res_original, originalStat, outer, numX, numY, numT, s0, t, alpha, nu, beta);

#if RUN_CPU_EXPERIMENTAL || RUN_ALL
        RunTestOnProgram<fun>("Simple Parallel Static", run_SimpleParallelStatic, res_original, originalStat, outer, numX, numY, numT, s0, t, alpha, nu, beta);
		RunTestOnProgram<fun>("Simple Parallel Dynamic", run_SimpleParallelDynamic, res_original, originalStat, outer, numX, numY, numT, s0, t, alpha, nu, beta);
#endif


		RunTestOnProgram<fun>("Interchanged", run_Interchanged, res_original, originalStat, outer, numX, numY, numT, s0, t, alpha, nu, beta);
#if RUN_CPU_EXPERIMENTAL || RUN_ALL
        RunTestOnProgram<fun>("Interchanged Optimized", run_InterchangedAlternative, res_original, originalStat, outer, numX, numY, numT, s0, t, alpha, nu, beta);
#endif  
		RunTestOnProgram<fun>("Parallel Interchanged", run_InterchangedParallel, res_original, originalStat, outer, numX, numY, numT, s0, t, alpha, nu, beta);
#if RUN_CPU_EXPERIMENTAL || RUN_ALL
        RunTestOnProgram<fun>("Parallel Interchanged Optimized", run_InterchangedParallelAlternative, res_original, originalStat, outer, numX, numY, numT, s0, t, alpha, nu, beta);
#endif  

        RunTestOnProgram<funT, 16>("Kernelized", run_Kernelized, res_original, originalStat, outer, numX, numY, numT, s0, t, alpha, nu, beta);
    }
