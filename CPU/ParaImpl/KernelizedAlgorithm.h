...
void rollback_Kernelized_Dist(const uint outer, const uint numT, 
    const uint numX, const uint numY, 
    const vector<REAL> myTimeline, 
    const vector<REAL> myDxx,
    const vector<REAL> myDyy,
...
void rollback_Kernelized_Dist_para(const uint outer, const uint numT, 
    const uint numX, const uint numY, 
    const vector<REAL> myTimeline, 
    const vector<REAL> myDxx,
    const vector<REAL> myDyy,
...
void rollback_Kernelized_Dist_Alt(const uint outer, const uint numT, 
    const uint numX, const uint numY, 
    const vector<REAL> myTimeline, 
    const vector<REAL> myDxx,
    const vector<REAL> myDyy,
...
void rollback_Kernelized_Dist_Alt_para(const uint outer, const uint numT, 
    const uint numX, const uint numY, 
    const vector<REAL> myTimeline, 
    const vector<REAL> myDxx,
    const vector<REAL> myDyy,
...
int   run_Kernelized_Rollback_Dist(  
                const uint   outer,
                const uint   numX,
                const uint   numY,
                const uint   numT,
    ...
    updateParams_Kernelized(alpha, beta, nu, numX, numY, numT, myX, myY, myTimeline, myVarX, myVarY);
    //cout << "Test6" << endl;
	rollback_Kernelized_Dist(outer, numT, numX, numY, myTimeline, myDxx, myDyy, myVarX, myVarY, u, v, a, b, c, y, yy, myResult);
...
int   run_Kernelized_Rollback_Dist_Parallel(  
                const uint   outer,
                const uint   numX,
                const uint   numY,
                const uint   numT,
    ...
    updateParams_Kernelized_para(alpha, beta, nu, numX, numY, numT, myX, myY, myTimeline, myVarX, myVarY);
    //cout << "Test6" << endl;
	rollback_Kernelized_Dist_para(outer, numT, numX, numY, myTimeline, myDxx, myDyy, myVarX, myVarY, u, v, a, b, c, y, yy, myResult);
...
int   run_Kernelized_Rollback_Dist_Alt(  
                const uint   outer,
                const uint   numX,
                const uint   numY,
                const uint   numT,
...
int   run_Kernelized_Rollback_Dist_Alt_Parallel(  
                const uint   outer,
                const uint   numX,
                const uint   numY,
                const uint   numT,
    ...
    updateParams_Kernelized_para(alpha, beta, nu, numX, numY, numT, myX, myY, myTimeline, myVarX, myVarY);
    //cout << "Test6" << endl;
	rollback_Kernelized_Dist_Alt_para(outer, numT, numX, numY, myTimeline, myDxx, myDyy, myVarX, myVarY, u, v, a, b, c, y, yy, myResult);
...
int   run_Kernelized_Rollback_Dist_Flat(  
                const uint   outer,
                const uint   numX,
                const uint   numY,
                const uint   numT,
